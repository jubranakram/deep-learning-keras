# -*- coding: utf-8 -*-
"""
Book: Advanced Deep Learning with Keras (Example, page: 31)
...
# function to automatically generate a rnn model based on user's input


def rnn_model(input_shape, number_of_hidden_layers, dropout_params,
              number_of_units, activation_type, kernel_regularizer,
              output_layer_nodes, output_layer_activation):
    ...
    for idx in range(number_of_hidden_layers):
        if (idx != 0 and (idx == number_of_hidden_layers-1)):
            model.add(krs.layers.SimpleRNN(units=number_of_units[idx],
                                           dropout=dropout_params[idx],
                                           activation=activation_type[idx],
                                           kernel_regularizer=kernel_regularizer[idx]))
        elif (idx != 0 and (idx < number_of_hidden_layers-1)):
            model.add(krs.layers.SimpleRNN(units=number_of_units[idx],
                                           dropout=dropout_params[idx],
                                           activation=activation_type[idx],
                                           kernel_regularizer=kernel_regularizer[idx],
...
if __name__ == '__main__':
    ...
    # create a cnn model
    model = rnn_model(input_shape, num_hidden_layers, dropout_params, num_units,
    ...
    for idx in range(len(axs)):
        ...
        axs[idx].axis('off')
    fig.suptitle('RNN Classification Result on MNIST Data Set')

    print(np.argmax(labels[22]), np.argmax(y_test[ind[22]]))
