# -*- coding: utf-8 -*-
"""
Book: Advanced Deep Learning with Keras (Example, page: 24)
...
def pre_process_train_set(in_data, max_value):
    '''This function reshapes and normalizes the input'''
    in_data = np.expand_dims(in_data, axis=-1)
...
# function to automatically generate a cnn model based on user's input


def cnn_model(input_shape, number_of_hidden_layers, dropout_params,
              number_of_filters, kernel_size, strides, padding, pooling,
    ...
    for idx in range(number_of_hidden_layers):
        if idx:
            model.add(krs.layers.Conv2D(filters=number_of_filters[idx],
            ...
            model.add(krs.layers.Conv2D(filters=number_of_filters[idx],
                                        ...
                                        kernel_regularizer=kernel_regularizer[idx]))
        if not (0 in np.array(pooling[idx])):
            model.add(krs.layers.MaxPool2D(pooling[idx]))
        if (idx == number_of_hidden_layers-1):
...
if __name__ == '__main__':
    ...
    x_train = pre_process_train_set(x_train, 255)
    x_validation = pre_process_train_set(x_validation, 255)
    x_test = pre_process_train_set(x_test, 255)
    ...
    # model parameters
    input_shape = np.append(image_dimensions, 1)
    num_hidden_layers = 2
    dropout_params = [0, 0.2]
    num_filters = [16, 32]
    kernel_size = [3, 3]
    strides = [1, 1]
    padding = 'same'
    ...
    # create a cnn model
    model = cnn_model(input_shape, num_hidden_layers, dropout_params, num_filters,
                      kernel_size, strides, padding, pooling, activation_type,
                      kernel_regularizer, output_layer_nodes, output_layer_activation)
    ...
    for idx in range(len(axs)):
        ...
        axs[idx].axis('off')
    fig.suptitle('CNN Classification Result on MNIST Data Set')
